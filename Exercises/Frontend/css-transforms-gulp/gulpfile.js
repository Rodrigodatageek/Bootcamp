...
function publishCss(done, for_production = false) {
    ...
    const css_entries = Object.values(entries.css).filter(entry => {
        if(!fs.existsSync(entry)) {
            console.log(chalk.red('Entry file missing:', entry));
            return false;
        }
        return true;
    })

    if (!css_entries.length) {
        return done();
    }
    let output_path = null;
    let pipeline = gulp.src(css_entries, {base: '.'})
...
function publishJs(done, for_production = false) {
    ...
    const js_entries = {};
    for (let key in entries.js) {
        if(!fs.existsSync(entries.js[key])) {
            console.log(chalk.red('Entry file missing:', entries.js[key]));
        } else {
            js_entries[key] = entries.js[key];
        }
    }

    if (!Object.values(js_entries).length) {
        return done();
    }
    ...
    let pipeline = gulp.src(Object.values(js_entries))
        // .pipe(cache('js'))
        .pipe(vinyl_named())
